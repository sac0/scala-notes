...
import fp.intro.iomonad.{IO,Monad,Free,unsafePerformIO}
...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    def runLog[O](src: Process[IO,O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO,O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req,recv) =>
            val next =
              try recv(Right(fp.intro.iomonad.unsafePerformIO(req)(E)))
    ...
    import fp.intro.iomonad.IO0.fahrenheitToCelsius
