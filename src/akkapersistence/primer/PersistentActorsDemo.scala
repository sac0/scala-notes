...
object PersistentActorsDemo extends App {
...
//  accountant ! PoisonPill
  ...
  * Recap
   ...
   *
   * onPersistRejected
   * called when journal fails to write an event
   * actor is resumed
   *
   * persistAll
   ...
   *
   * never call persist from the future
   *
   * shutdown with custom messages not PoisonPill
   */
}
