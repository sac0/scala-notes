...
object ActorInfo extends App {
  /**
   * ACtor instance
   * has method
   * has internal state
   *
   * It has incarnation or actorref
   *
   * created by actorOf
   * has mailbox and receives messages
   * contains an actor instance and only one
   * contains an uuid
   *
   *
   * actor path
   *
   * may or may not have an actor ref here
   *
   *
   * Actor life cycle can be
   * started
   * suspended
   * resumed
   * restarted
   * stopped
   *
   * start = create an new actor ref with an uuid at a given path
   * suspend = create an actor ref and enqueue but not process more messages
   * resume = actor ref will continue processing more messages
   *
   * restarted = suspend | old instance calls preRestart | replace actor instance | new instance calls postRestart | resume
   * internal state is destroyed on restart
   *
   * stopped calls postStop
   * all watching actors receive Terminated(ref)
   * Another actor might start here if needed
   */
