...
trait GuitarStoreJsonProtocol extends DefaultJsonProtocol {
  ...
  // format 3 since we have 3 parameters in the case class after importing spray json
  // stick to case classes for json formatters
  implicit val guitarFormat: RootJsonFormat[Guitar] = jsonFormat3(Guitar)
}

object RestAPIs extends App with GuitarStoreJsonProtocol {
  ...
  val requestHandler: HttpRequest => Future[HttpResponse] = {
    ...
    /**
     * If we do not handle this 404. Will be counted as backpressure to tcp level
     */
